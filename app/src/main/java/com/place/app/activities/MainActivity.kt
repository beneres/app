...
import com.place.app.fragment.LocationDetails
import com.place.app.data.RetroClient
import com.place.app.data.model.Location
import com.place.app.helper.Helper
import com.place.app.helper.Helper.placeOfColors
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(com.place.app.R.layout.activity_main)
        loadPlacesData()

        //Initialize fused Location
        client = LocationServices.getFusedLocationProviderClient(this)
        supportMapFragment = supportFragmentManager.findFragmentById(com.place.app.R.id.google_map) as SupportMapFragment?
        //check permission
                    ...
                    for (i in locations.indices) {
                        ...
                        supportMapFragment!!.getMapAsync { googleMap ->
                            ...
                            googleMap.addMarker(MarkerOptions().position(latLng).title(locationItem.name)
                                    ...
                                    .icon(getMarkerIcon(placeOfColors[Random().nextInt(placeOfColors.size)])))?.tag = locationItem.name
                            ...
                            googleMap.setOnMarkerClickListener { marker: Marker ->
                                for (i in locations.indices) {
                                    if (locations[i]!!.name == marker.tag) {
                                        ...
                                        currentLocationID = i
                                        LocationDetails().show(supportFragmentManager, "Details_Location_Fragment_TAG")
                                        break
                                    }
                                }
                                false
    ...
    protected fun loadPlacesData() {
         ...
         */
        if (Helper.checkConnection(applicationContext)) {
            ...
            /**
             * Enqueue Callback will be call when data response...
             */
            call!!.enqueue(object : Call<List<Location?>>, Callback<List<Location?>?> {
                ...
                override fun onResponse(call: Call<List<Location?>?>, response: Response<List<Location?>?>) {
                    ...
                    if (response.isSuccessful) {
                         ...
                         *  Got Data
                         */
                        Toast.makeText(this@MainActivity, "Got Data from Json ", Toast.LENGTH_SHORT).show()

                        locations= (response.body() as ArrayList<Location>?)!!
                        // adapter.notifyDataSetChanged();
                        Helper.locations =locations
                        currentLocation
                        // initLocation();
                    } else {
                ...
                override fun onFailure(call: Call<List<Location?>?>, t: Throwable) {
                    Toast.makeText(this@MainActivity, "Failure! : not data Data", Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                }
